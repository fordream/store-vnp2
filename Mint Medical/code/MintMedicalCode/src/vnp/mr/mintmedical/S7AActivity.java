...
public class S7AActivity extends FragmentActivity implements
	...
	private List<Object> getlData() {
		...
		if (clinic != null) {
			...
			if (clinic.phone != null) {
				url = "tel:"
						+ clinic.phone.replace("(", "").replace(")", "")
								.replace("-", "").replace(" ", "");
			}
			intentCall = new Intent(Intent.ACTION_CALL, Uri.parse(url));
		}

		S7Item item = new S7Item(R.drawable.s7a2, "Call this Office", "",
				intentCall);
		item.phone = clinic.phone;
		data.add(item);
		...
		data.add(new S7Item(R.drawable.s7a3, "Driving Directions", "",
				intentDirections));
	...
	protected void onResume() {
		...
		resize.resizeSacle(findViewById(R.id.s7a_content1),
				MintUtils.WIDTH_ITEM,
				android.widget.LinearLayout.LayoutParams.WRAP_CONTENT);
		resize.resizeSacle(findViewById(R.id.s7aitem_text),
				MintUtils.WIDTH_ITEM, 70);
	...
	private class AboutActivityItemView extends CustomLinearLayoutView {
		...
		public void init(int res) {
			...
			avatarView.setType(AvatarView.S7A);
		}

		@Override
		protected void onAttachedToWindow() {
			super.onAttachedToWindow();
			resize.resizeSacle(findViewById(R.id.s7itemview_main),
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		try {
				...
				if (!dbUserLogin.isLogin()) {
					DialogInterface.OnClickListener onClick = new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							MintUtils.toastMakeText(S7AActivity.this, "Login success");
						}
					};
			...
			MintUtils.showDialog(this, "Can not call phone : " + item.phone);
		}
	}
