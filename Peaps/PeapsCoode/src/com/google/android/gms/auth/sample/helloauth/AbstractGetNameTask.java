...
public class AbstractGetNameTask extends AsyncTask<Void, Void, Void> {
	...
	AbstractGetNameTask(Context activity, String email, String scope,
			IAbstractGetNameTaskCallBack iAbstractGetNameTaskCallBack) {
		this.mActivity = activity;
		this.mScope = scope;
		this.mEmail = email;

		this.iAbstractGetNameTaskCallBack = iAbstractGetNameTaskCallBack;
		iAbstractGetNameTaskCallBack.setEmail(this.mEmail);
	}
	...
	protected void onError(String msg, Exception e) {
		...
		iAbstractGetNameTaskCallBack.showErrorMessage(msg); // will be run in UI
															// thread
	}
	...
	private void fetchNameFromProfileServer() throws IOException, JSONException {
		...
		if (sc == 200) {
			...
			iAbstractGetNameTaskCallBack.handlesuccess(name, response);
			is.close();
			return;
		} else if (sc == 401) {
